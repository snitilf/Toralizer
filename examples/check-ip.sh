...
echo "using: $CURL"
echo ""
echo "your real ip:    $($CURL -s http://httpbin.org/ip 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo 'failed')"
echo "tor exit node:   $(../toralize $CURL -s http://httpbin.org/ip 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo 'failed')"
echo ""
